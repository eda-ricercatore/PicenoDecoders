...
$gh_flag = $ARGV[0];
print "Th selected option/flag is:::",$gh_flag,"==\n";
...
##	@row_indices1 = sort(@row_indices);
	...
	@row_indices1 = @row_indices;
	# Bubble sort this!
	for($bs1=0; $bs1<($#row_indices1); $bs1++) {
		for($bs2=($#row_indices1); $bs2>$bs1; $bs2--) {
			if(@row_indices1[($bs2-1)] > @row_indices1[$bs2]) {
				$temp = @row_indices1[$bs2];
				@row_indices1[$bs2] = @row_indices1[($bs2-1)];
				@row_indices1[($bs2-1)] = $temp;
			}
#print "row_indices1[$bs2]",@row_indices1[$bs2],":::\n";
#print "row_indices1[($bs2-1)]",@row_indices1[($bs2-1)],":::\n";
		}
	}

print "---Sorted row indices---\n";
foreach $i (@row_indices1) {
	print $i," ";
}
print "\n";
...
#pop(@submatrix_i3);
#print ip2 @ipln[pop(@submatrix_i3)]," ";
	...
	foreach $ipline (@ip_matrix) {
		...
		if(($ip_cols/2)<$ip_rows) {
#			if(($ip_rows%2)==1) {
#			if((($ip_cols+1)/4)<$ip_rows) {
			if((($ip_cols-$ip_rows+1)%2)==0) {
				print ip2 @ipln[(@submatrix_i3[@submatrix_i3-1])]," ";
			}
			for($i=0; $i<(@submatrix_i3); $i++) {
				print ip2 @ipln[$i]," ";
			}
		}else{
...
shift(@submatrix_i3);
	# Enumerate each row, and print its columns acording to
	# the sorted indices in the array submatrix_i
#	foreach $ipline (reverse(@ip_matrix)) {
	foreach $ipline (@ip_matrix) {
		@ipln = split('\s',$ipline);
...
#			}
#		}else{
			foreach $i (reverse(@submatrix_i3)) {
##			foreach $i (@submatrix_i3) {
				print ip2 @ipln[$i]," ";
			}
#		}	
...
####		for($i=(@submatrix_i3-1); $i>=0; $i--) {
#			print ip2 @ipln[(@submatrix_i3[$i])]," ";
####			print ip2 @ipln[$i]," ";
####		}
#		for($i=0; $i<@submatrix_i3; $i++) {
#			print ip2 @ipln[(@submatrix_i3[$i])]," ";
...
print "=============The selected option/flag is:::",$gh_flag,"==\n";
	if($gh_flag eq "-g") {
		# Obtain the data values from the P' matrix
		for($dv=0; $dv<$sz_data; $dv++) {
			print dec "		d[",$dv,"] = cx1[";
...
print "++++The selected option/flag is:::",$gh_flag,"==\n";
		# Obtain the data values from the P' matrix
		#################for($dv=($sz_data-1); $dv>=0; $dv--) {
		for($dv=($num_cols-$sz_data); $dv<($num_cols); $dv++) {
			print dec "		d[",($sz_data-$num_cols+$dv),"] = cx1[";
			#print dec @submatrix_i[$dv],"];\n";
			#print dec ($n_k+$dv),"];\n"
			print dec $dv,"];\n";
			#print dec @submatrix_i[$n_k+$dv],"];\n";
