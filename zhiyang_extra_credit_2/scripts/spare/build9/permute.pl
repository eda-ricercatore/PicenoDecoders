...
print "Value of max:",$max,":::\n";
			if((@submatrix_i2[$b] > $max) && (@submatrix_i2[$b]<@sort_subm[$a])) {
#			if(@submatrix_i2[$b]<@sort_subm[$a]) {
			
#				if(($max > @submatrix_i2[$b]) && (@submatrix_i2[$b]<@sort_subm[$a])) {
				$max = @submatrix_i2[$b];
				$max_index=$b;
...
print "\n";
	...
	for($a=0; $a<=$ip_cols; $a++) {
		if($a > 9) {
			$s_elem=shift(@sort_subm);
			$s_elem=$s_elem . shift(@sort_subm);
		}else{
			$s_elem=shift(@sort_subm);
		}
		unshift(@sort_subm,$s_elem);
print "S_Elem:",$s_elem,"###\n";		
		#if(($a == @sort_subm[0]) && (@sort_subm>1)) {
